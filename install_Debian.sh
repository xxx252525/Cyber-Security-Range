...
# 安装和配置HTTP服务
execute_command sudo apt install -y httpd
...
# 安装PHP和相关模块
execute_command sudo apt install -y php php-json php-fpm php-gd php-xml
execute_command sudo apt install -y php php-pear php-devel
php --version

# 安装Java OpenJDK
execute_command sudo apt install -y java-21-openjdk
java --version
